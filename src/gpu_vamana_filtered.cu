...
__device__ __forceinline__ void canonical_label_balanced_shortlist_warp(
    uint32_t row, uint32_t R, uint32_t begin, uint32_t end,
    const uint32_t *candidate_ids, const float *candidate_distances,
    const uint32_t *offsets, const uint32_t *labels, uint8_t *state, uint32_t lane)
{
    if (g_filtered_geometric_only_prune != 0 || end - begin <= R)
        return;
    const uint32_t label_begin = offsets[row];
    uint32_t label_count = offsets[row + 1] - label_begin;
    if (label_count > 16)
        label_count = 16;
    uint32_t keep_count = 0;
    const uint32_t quota = g_filtered_balance_per_label;



    for (uint32_t kept = 0; kept < quota && keep_count < R; ++kept)
    {
        for (uint32_t q = 0; q < label_count && keep_count < R; ++q)
        {
            const uint32_t label = labels[label_begin + q];
            float best_distance = FLT_MAX;
            uint32_t best_id = UINT32_MAX, best_position = UINT32_MAX;
            for (uint32_t p = begin + lane; p < end; p += 32)
            {
                const uint32_t id = candidate_ids[p];
                if (state[p] != 0 || !point_has_exact_label_device(id, label, offsets, labels))
                    continue;
                const float distance = candidate_distances[p];
                if (distance < best_distance ||
                    (distance == best_distance && (id < best_id ||
                     (id == best_id && p < best_position))))
                {
                    best_distance = distance; best_id = id; best_position = p;
                }
            }
            for (uint32_t offset = 16; offset > 0; offset >>= 1)
            {
                ...
                if (other_distance < best_distance ||
                    (other_distance == best_distance && (other_id < best_id ||
                     (other_id == best_id && other_position < best_position))))
                {
                    best_distance = other_distance; best_id = other_id; best_position = other_position;
                }
            }
                                      ...
                                      uint32_t *new_degrees, uint8_t *row_changed)
{
    ...
    __syncwarp();
    canonical_label_balanced_shortlist_warp(
        row, R, work_base, work_base + candidate_count, work, work_distances,
        offsets, labels, removed, lane);
    uint32_t selected_count = 0;
                                                ...
                                                uint32_t *d_handoff_degrees = nullptr)
{
    ...
    const uint32_t tau_F = env_u32_clamped("DISKANN_GPU_FILTERED_CANONICAL_TAU_F", 8, 1, R);
    ...
    const bool enable_coverage_repair = true;
    ...
    printf("[gpu_vamana_filtered_navigation_policy] true_ordinary_round0=%u pure_ordinary_round0=%u "
           "ordinary_pool=%u ordinary_steps=%u posting_anchor=%u additional_round0_anchor=%u "
           "bridge_work_reserve=%u\n",
           true_ordinary_round0, pure_ordinary_round0, ordinary_pool, true_ordinary_steps,
    ...
    {
        const uint32_t *round_ids = round == 0 ? nullptr : d_active_ids;
        ...
        if (round > 0)
        {
            ...
            CUDA_CHECK_FILTERED(cub::DeviceSelect::Flagged(d_select_temp, select_temp_bytes,
                                                            d_all_task_ids, d_task_flags,
                                                            d_active_task_ids, d_selected_count, total_labels));
            CUDA_CHECK_FILTERED(cudaMemcpy(&active_task_count, d_selected_count, sizeof(uint32_t),
...
#undef CANONICAL_LAUNCH_WORK_DISTANCE
        ...
        canonical_mark_deficient<<<(N + 255) / 256, 256>>>(
            N, R, tau_F, d_graph_b, d_degree_b, d_offsets, d_labels, d_label_sizes,
            num_labels, universal_label, d_deficient);
        canonical_combine_frontier<<<(N + 255) / 256, 256>>>(
            N, d_changed, d_touched, d_deficient, d_active);
        uint32_t changed_count = 0, deficient_count = 0, next_active_count = 0;
        ...
        CUDA_CHECK_FILTERED(cub::DeviceSelect::Flagged(d_select_temp, select_temp_bytes, d_all_ids, d_deficient,
                                                        d_deficient_ids, d_selected_count, N));
        CUDA_CHECK_FILTERED(cudaMemcpy(&deficient_count, d_selected_count, sizeof(uint32_t), cudaMemcpyDeviceToHost));
        CUDA_CHECK_FILTERED(cub::DeviceSelect::Flagged(d_select_temp, select_temp_bytes, d_all_ids, d_active,
                                                        d_active_ids, d_selected_count, N));
        CUDA_CHECK_FILTERED(cudaMemcpy(&next_active_count, d_selected_count, sizeof(uint32_t), cudaMemcpyDeviceToHost));
        printf("[gpu_vamana_filtered_canonical_round] round=%u kind=%s alpha=%.3f active=%u tasks=%u changed=%u touched=%u reverse_proposals=%u deficient=%u next_active=%u\n",
               round, round == 0 ? "full" : "delta", round_alpha, active_count, active_task_count,
               changed_count, touched_count, round_reverse_proposals, deficient_count, next_active_count);
                                          ...
                                          const char *type_name)
{
    ...
    else if (run_canonical_v2 && run_refine_vnew2 && canonical_device_handoff)
    {
        ...
        if constexpr (std::is_same<DataT, uint8_t>::value)
            ret = gpu_vamana_vnew2_build_device(h_data, num_points, dim, R, L, gpu_backbone_C, STEPS,
                                                 &d_handoff_data, &d_handoff_graph, &d_handoff_degree);
        else
            ret = gpu_vamana_vnew2_build_device_float(h_data, num_points, dim, R, L, gpu_backbone_C, STEPS,
                                                       &d_handoff_data, &d_handoff_graph, &d_handoff_degree);
        printf("[gpu_vamana_filtered] backbone_handoff=device_resident\n");
    }
